...
Merge new annotations into the corpus. Handles:
- pipe-separated annotation strings (utterance|context|maxim|violation_type)
- annotation CSVs paired with scraped CSVs (using positional matching)
- annotation CSVs with full utterance/context columns
- deduplication by (utterance, context)
- violation_type normalization (flout -> flouting, sincere -> none, etc.)
- printing distribution before/after for sanity checking

i wrote this because i wrote the same merge script eight times and
realized maybe that was a sign. now there's one function.

Usage as a library:
    from merge_corpus import merge_pipe_data, merge_annotated_csv

    # pipe-separated string with full data
    merge_pipe_data(\"\"\"
    Hello.|Hi there.|Cooperative|none
    Fine.|How are you really?|Quantity|violating
    \"\"\")

    # annotated CSV with utterance/context/maxim/violation_type columns
    merge_annotated_csv("data/raw/reddit_aita_annotated.csv")

    # annotated CSV that needs to be matched against a scraped CSV by row order
    # (used when you only annotated the gold columns and need to merge with the
    # scraped utterance/context)
    merge_with_scraped(
        scraped_path="data/raw/reddit_aita.csv",
        annotation_pipe_string=\"\"\"...\"\"\",
    )

Usage as a script:
    python merge_corpus.py --csv path/to/annotated.csv
    python merge_corpus.py --scraped path/to/scraped.csv --annotations path/to/annotations.csv
"""
